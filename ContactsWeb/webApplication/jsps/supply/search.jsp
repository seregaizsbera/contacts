...
--%><!-- jsps/suplly/search.jsp { --><%--
 ...
 --%><body onload="setFocus('searchForm', 'name')"><%--
  ...
  --%><jstl:if test="${not empty supplySearchParameters}"><%--
   --%><table align="right"><%--
    --%><tr><%--
   ...
   --%></table><%--
  --%></jstl:if><%--
  --%><jstl:if test="${not empty supplies}"><%--
   --%><jsp:include page="/include/supply/search_results.jsp" flush="true"/><%--
  --%></jstl:if><%--
  --%><jstl:if test="${empty supplies and not empty supplySearchParameters}"><%--
  ...
  --%><form name="searchForm" method="GET" action="<%=request.getContextPath()%>/controller"><%--
   ...
   --%><table width="100%" class="form" frame="border"><%--
    --%><tr><%--
    ...
    --%><tr><%--
     ...
     --%><td><%--
       ...
       --%><select id="metroId" name="metro" class="elem" tabindex="22"><%--
        --%><option value="">Не имеет значения</option><%--
        --%><logic:iterate name="inquire_metro_stations_1" id="station"><%--
         --%><option value="<jstl:out value="${station.id}"/>"<jstl:if test="${supplySearchParameters.metro == station.id}"> selected</jstl:if>><jstl:out value="${station.name}"/></option><%--
        --%></logic:iterate><%--
       --%></select><%--
