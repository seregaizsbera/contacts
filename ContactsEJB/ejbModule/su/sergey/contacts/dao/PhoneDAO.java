...
public final class PhoneDAO extends AbstractDAO {
    ...
    public Integer create(PhoneCreateInfo value) throws DAOException {
        ...
        try {
            conn = getConnection();
            pstmt = conn.prepareStatement("INSERT INTO phones (phone, type, basic) VALUES (?, ?, ?)");
            int index = 1;
    ...
    public PhoneData find(PhoneHandle handle) throws DAOException {
        ...
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        String query = "SELECT id, phone, type, basic FROM phones WHERE id = ?";
    ...
    public void update(PhoneHandle handle, PhoneUpdateInfo value) throws DAOException {
        Connection conn = null;
        PreparedStatement pstmt = null;
        String query = "UPDATE phones SET phone = ?, type = ?, basic = ? WHERE id = ?";
        try {
            conn = getConnection();
            pstmt = conn.prepareStatement(query);
            int index = 1;
